...
#define APIKey "H26UVWP20AEFY28B"
/* USER CODE END PM */
...
char cmd_buffer[100];
char THINGSPEAK_REQUEST[150]; // Augmenté pour plus de sécurité

uint16_t ADC_Value[3];
int systemState = 0;
volatile int BOOL1 = 0;
...
void ESP_Init_Station_Client(void) {
    HAL_UART_Transmit(HTERM, (uint8_t*)"WiFi Init Start...\r\n", 20, 1000);
    ...
    HAL_UART_Transmit(ESP_UART, (uint8_t*)cmd_cifsr, strlen(cmd_cifsr), 2000);
    HAL_Delay(2000);
}

void Transmission_Cloud_THINGSPEAK(void)
{
    HAL_UART_Transmit(HTERM, (uint8_t *)&Cloud_BEGIN[0], strlen(Cloud_BEGIN), 1000);

    sprintf(cmd_buffer, "AT+CIPSTART=\"TCP\",\"184.106.153.149\",80\r\n");
    HAL_UART_Transmit(ESP_UART, (uint8_t *)&cmd_buffer[0], strlen(cmd_buffer), 1000);
    HAL_Delay(1000);
    ClearBuffer(cmd_buffer, sizeof(cmd_buffer));

    sprintf(THINGSPEAK_REQUEST, "GET /update?api_key=%s&field1=%hu&field2=%hu&field3=%hu\r\n",
    ...
    HAL_UART_Transmit(ESP_UART, (uint8_t *)&THINGSPEAK_REQUEST[0], strlen(THINGSPEAK_REQUEST), 1000);
    HAL_Delay(1000);
...
int main(void)
{
  ...
  HAL_UARTEx_ReceiveToIdle_IT(ESP_UART, (uint8_t *)Buffer_rec, 3000);
